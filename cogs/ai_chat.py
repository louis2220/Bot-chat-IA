...
class AIChat(commands.Cog):
    def __init__(self, bot):
        ...
        if not self.api_key:
            ...
            logger.info(f"✅ GEMINI_API_KEY carregada ({self.api_key[:8]}...)")
    ...
    async def get_ai_response(self, user_message: str, user_id: int, user_name: str) -> str:
        try:
            ...
            # Loga o erro COMPLETO nas logs do Railway
            logger.error(f"Erro Gemini: {type(e).__name__}: {e}")
            error = str(e).lower()
            if "quota" in error or "limit" in error or "429" in error:
                return "Ei, tô com muita demanda agora! 😅 Tenta de novo em uns segundinhos, mano!"
            if "api_key" in error or "invalid" in error or "401" in error or "403" in error:
                return "Eita, tem um problema com minha configuração! 😬 Chama o admin do servidor."
            if "blocked" in error or "safety" in error:
                return "Hmm, não consigo responder isso não, mano! 😅 Tenta perguntar outra coisa."
